...
Companion de [`SPEC.md`](SPEC.md). Contrat de réversibilité par tool : pour chaque write-tool d'inc.3, quelle op le journal enregistre, quel `prevState` il capture, et quel inverse le rewind rejoue. Règle absolue, non négociée : **aucun inverse n'effectue de hard-delete** — tout inverse est un re-archivage (`archivedAt`), une restauration de champs (`prevState`), ou un retrait soft.
...
| `op` | `created` \| `merged` \| `reactivated` \| `rewind`. Détermine l'inverse. |
| `prevState?` | État antérieur des champs touchés — REQUIS pour `merged`/`reactivated`, omis pour `created`. |
...
| `composeMessage` | `created` (brouillon, `statut = "brouillon"`) | — | Retrait **soft** du brouillon (chemin soft à choisir : `archivedAt` sur `messages` ou statut terminal `"annule"` — voir Assumptions de SPEC.md). Jamais de `DELETE`. |
...
- **`created` ⇒ archiver ; `merged`/`reactivated` ⇒ restaurer `prevState`.** Ne jamais archiver un contact dont le tour n'a fait que modifier des champs : il préexistait, le rewind doit le rendre identique à avant le tour, pas le faire disparaître. C'est le cœur de CAP-3.
